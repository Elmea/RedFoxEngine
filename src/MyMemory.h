...
void *MyTempAlloc(Memory *memory, size_t size);
...
fileResource FileResourceInit(char *fileName, Memory *m);
MyString OpenAndReadEntireFile(const char *filePath, Memory *memory);
fileResource *LoadFile(fileResource *result, Memory *memory);
MyString initStringChar(char *str, u64 n, Memory *memory);
...
static int StringsAreEqual_C(MyString a, char *str, char *delimiter)
{
    int i = 0;
...
static MyString initString(u64 n, Memory *memory)
{
    ...
    result.size = 0;
    result.capacity = n;
    result.data = (u8 *)MyMalloc(memory, n);
    return (result);
}

static MyString initStringChar(char *str, u64 n, Memory *memory)
{
    MyString result = {0};
...
static MyString OpenAndReadEntireFile(const char *filePath, Memory *memory)
{
    ...
    if (File)
    {
        ...
        DWORD test = 0;
        ReadFile(File, result.data, result.size, &test, NULL);
...
static
char *my_strncpy_s(char *dest, u64 dest_size, char *src, u64 src_size)
{
    int i = 0;
...
static fileResource
FileResourceInit(char *fileName, Memory *m)
{
    fileResource result;
...
*LoadFile(fileResource *result, Memory *memory)
{
    ...
    if (result->handle != INVALID_HANDLE_VALUE)
    {
        ...
        result->data = (u8 *)MyMalloc(memory, result->size);
