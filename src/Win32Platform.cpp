...
//				SetCapture(m_window);
			...
			case WM_KEYDOWN:
			{
				...
				// NOTE(V. Caraulan): the comments are for US/ISO international keys https://www.win.tue.nl/~aeb/linux/kbd/scancodes-1.html
				switch (scanCode)
				{
					case 0x01: //Escape
					{
...
void Platform::GetWglFunctions(void)
{
	...
	{
		{ __debugbreak(); return; }//FatalError("Cannot set OpenGL pixel format for dummy window!");
	}
	...
	{
		{ __debugbreak(); return; } //("OpenGL does not support WGL_ARB_extensions_string extension!");
	}
